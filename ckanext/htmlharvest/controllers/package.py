...
from ckanext.htmlharvest.plugin import DATASET_TYPE_NAME
...
class CustomHtmlHarvestController(PackageController):


    def __before__(self, action, **params):

        super(CustomHtmlHarvestController, self).__before__(action, **params)

        c.dataset_type = DATASET_TYPE_NAME

    def new_custom_metaharvester(self):
        return render('htmlharvest1.html')
	...
	#    couch = pycouchdb.Server(SERVER_URL)
        ...
        log.info('htmlharvest: action triggered: %r', data)
