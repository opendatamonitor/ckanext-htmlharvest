...
#        p.toolkit.add_resource('public/ckanext/harvest/javascript', 'harvest-extra-field')
    ...
    def after_create(self, context, data_dict):
        if 'type' in data_dict and data_dict['type'] == DATASET_TYPE_NAME and not self.startup:
            log.info('HTMLHarvest_form: Nothing important')

    def before_map(self, m):
        ''' Called before routes map is setup. '''
        controller = 'ckanext.htmlharvest.controllers.package:CustomHtmlHarvestController'
        m.connect('new_custom_metaharvester', '/' + DATASET_TYPE_NAME + '/htmlharvest_form_first',
